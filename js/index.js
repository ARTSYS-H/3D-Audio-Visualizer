/**
Audio Visualizer by Lucas Hadey.
http://www.lucashadey.com/
*/
...
AudioVisualizer.prototype.createBars = function () {
    ...
    for (var i = 0; i < this.numberOfBars; i++) {
        ...
        var material = new THREE.MeshPhongMaterial({
            color: this.getRandomColor(), /*'darkorange',*/
            ambient: 0x808080
        });
        ...
        /*if(i < this.numberOfBars/3){
          this.bars[i].position.set(i*2 - this.numberOfBars/2, 0, 0);
          //this.bars[i].material.color.set('green');
        }else if (i < (this.numberOfBars/3)*2) {
          this.bars[i].position.set((i - this.numberOfBars/3)*2 - this.numberOfBars/2, 0, 1.5);
          //this.bars[i].material.color.set('yellow');
        }else {
          this.bars[i].position.set((i - this.numberOfBars/1.5)*2 - this.numberOfBars/2, 0, 3);
          //this.bars[i].material.color.set('red');
        }*/
...
AudioVisualizer.prototype.setupAudioProcessing = function () {
    ...
    this.javascriptNode.onaudioprocess = function () {
        ...
        for (var i = 0; i < visualizer.numberOfBars; i++) {
            ...
            //visualizer.bars[i].position.setY(value/2.5);
            visualizer.bars[i].scale.setZ(value);
        }
    }
};
...
AudioVisualizer.prototype.getAudio = function () {
    ...
    request.open("GET", "4. Journey of King.mp3", true);
...
AudioVisualizer.prototype.handleDrop = function () {
    ...
    document.body.addEventListener("drop", function (e) {
        ...
        $("#guide").text("Joue " + fileName);

        var fileReader = new FileReader();

        fileReader.onload = function (e) {
            visualizer.sourceBuffer.stop();
            visualizer.setupAudioProcessing();
